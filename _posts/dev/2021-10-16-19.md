...
버킷의 크기는 12이고, 3의 배수들이 키(key)로 들어온다고 하면 위와 같다. 이는 3이 12의 약수이기 때문에 발생하는 일로, 특정 인덱스에 몰리고, 다른 인덱스는 비어있음을 알 수 있다.
...
버킷의 크기는 11이고, 3의 배수들이 키(key)로 들어온다고 하면 위와 같다. 나머지 연산을 통해 인덱스를 결정하기 때문에, 버킷의 크기를 소수로 하면 골고루 배치할 수 있다. 나머지 연산에 의한 충돌을 최대한 회피한다.
...
- 해시테이블을 만들 때, 나머지 연산을 활용하며 버킷의 크기는 소수여야 한다.
