...
공부하면서 이런저런 게시글들을 많이 보았다. [토큰을 쿠키에 저장하느냐, 로컬 스토리지에 저장하느냐](https://velog.io/@0307kwon/JWT%EB%8A%94-%EC%96%B4%EB%94%94%EC%97%90-%EC%A0%80%EC%9E%A5%ED%95%B4%EC%95%BC%ED%95%A0%EA%B9%8C-localStorage-vs-cookie) 선택할 수 있고, [HTTP 헤더의 `Authorization`에 저장하느냐, `X-Auth-Token`에 저장하느냐](https://stackoverflow.com/questions/39017297/when-looking-at-the-differences-between-x-auth-token-vs-authorization-headers-w) 선택할 수 있다.

### 장점

1. 토큰을 URL 파라미터(url-safe) 또는 HTTP 헤더로 사용 가능하다.
2. 인증 서버/DB가 필요없다. >> 서버 자원 절약
3. 서버 분산, 확장에 용이하다.

### 단점

1. JWT에 데이터를 담을 수 있지만, 당연히 담을수록 **길이**는 길어진다.
2. 토큰을 매번 HTTP 헤더에 붙혀야하는데 이는 **트래픽의 크기**에 영향을 미친다.
3. 서버 측에서 데이터가 변경되어도 클라이언트에 저장된 이미 발급된 토큰이므로, **변경할 수 없다.**
...
1, 2번 문제는 어쩔 수 없는 문제점이라고 생각했다. 3번 문제는 만료기한을 짧게 설정하고 토큰을 자주 발급하면 어느정도 해결될 듯 했다.

4번 문제는 참조 링크를 쭉 읽고 '아 그렇구나'하고 넘어갔다. 여기서 내가 가져갈 수 있는 것은 **보안 관련한 쿠키 설정**(HttpOnly, Secure, SameSite, cookie prefixes)을 추가하여 보안을 강화할 수 있다는 것이었다.

## JWT (JSON Web Token)

위에서 설명한 토큰이 바로 JWT이다.([RFC7519](https://datatracker.ietf.org/doc/html/rfc7519)) 회원 인증을 하는데 주로 사용하며, 토큰을 이용하여 검증하므로 **서버는 세션을 관리하지 않는다.** 이 때문에 서버는 자원을 절약할 수 있다.
