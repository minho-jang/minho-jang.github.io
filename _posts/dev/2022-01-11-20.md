...
tags:
  ...
  - java
header:
  teaser: /assets/images/dev/spring-web-flow.png
---
...
여기서 의문이 들 것이다.~~(아닐 수도 있고)~~

스프링에서 컨트롤러를 만들어본 사람이라면 메서드에서 필요로 하는 인자도 다양하고, 반환하는 타입도 다양한 것을 알 것이다.
...
`@RestController`는 `@ResponseBody`를 포함하고 있는데, **`@ResponseBody`가 적용된 컨트롤러는 `handle()` 호출 결과가 null 이고 반환 시점에 이미 response에 값이 담겨있다.** `@ResponseBody`만 그런건 아니고, 이미 응답 처리가 완료되었다면 `handle()` 호출 결과는 `null`이다.
...
그럼 이러한 경우는 언제 response가 만들어지는가? 핸들러 어댑터에서 `HandlerMethodReturnValueHandler`가 동작하여 반환 결과를 조작하는데, 위에서 컨트롤러의 반환 타입이 다양할 수 있는 이유가 이 때문이다. RestController는 핸들러 어댑터가 핸들러를 호출 결과를 다루기 위해 `HandlerMethodReturnValueHandler` 중 `RequestResponseBodyMethodProcessor.handleReturnValue()`가 실행되고 메시지 컨버터에 의해 body에 결과를 담는다.
...
`doDispatch()`는 이 함수를 마지막으로 호출하고 끝난다. 물론 위 코드는 `try/catch/finally` 블록이 생략되어있다.
